...
describe('OptionsController.', function () {
    describe('Not initialized.', function () {
        it('Any option should be undefined', function () {
            let defaults = defaultOptions.get();
            Object.keys(defaults).forEach(key => assert.isUndefined(PluginController.get(key)));
            assert.isUndefined(PluginController.get('otherKey'));
        })
    });
